...
export default function OrderDetailsPage({ orderId, driverId, onBack }: OrderDetailsPageProps) {
  ...
  const acceptOrderMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "تم تعيين الطلب لك. توجه الآن للمتجر لاستلامه."
      });
      onBack();
    },
    onError: (error: Error) => {
  ...
  const getStatusText = (status: string) => {
    const statusMap: Record<string, string> = {
      ...
      assigned: 'مُعين لك (توجه للمتجر)',
      accepted: 'مُعين لك (توجه للمتجر)',
  ...
  const getNextStatus = (currentStatus: string) => {
    ...
    return statusFlow[currentStatus] || 'picked_up';
  };
  ...
  const getNextStatusLabel = (currentStatus: string) => {
    const labels: Record<string, string> = {
      ...
      pending: 'قبول الطلب',
      preparing: 'جاهز للاستلام بالمطعم',
      ready: 'استلام الطلب من المتجر',
      ...
      <main className="max-w-4xl mx-auto p-4 pb-20">
            ...
            <div className="flex gap-4 text-sm">
              <div>
                <p className="text-gray-600">
                  عمولتك ({order?.driverCommissionRate ? `${(parseFloat(String(order.driverCommissionRate)) || 0).toFixed(0)}%` : '—'})
                </p>
            ...
            <div className="flex justify-between items-center gap-4">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() => setCallDialog({
                    isOpen: true,
                    name: order.adminName || 'مدير العمليات (المناوب)',
                    role: 'admin',
                    phone: order.adminPhone || '+967777777777',
            ...
            <div className="flex justify-between items-start">
              ...
              {order.restaurantPhone && (
                <Button
                  ...
                  onClick={() => setCallDialog({
                    isOpen: true,
                    name: order.restaurantName || 'المتجر / المطعم',
                    role: 'restaurant',
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4 space-y-3">
          ...
          {order.driverId === driverId && !['delivered', 'cancelled'].includes(order.status) && (
            <div className="space-y-2">
              <Button
                ...
                className="w-full h-12 gap-2 bg-blue-600 hover:bg-blue-700 text-white text-lg font-bold"
              >
                <CheckCircle className="h-5 w-5" />
                {updateOrderStatusMutation.isPending ? 'جاري التحديث...' : getNextStatusLabel(order.status)}
              </Button>

              {order.status !== 'on_way' && order.status !== 'on_the_way' && (
                <Button
                  onClick={() => updateOrderStatusMutation.mutate('on_way')}
                  disabled={updateOrderStatusMutation.isPending}
                  variant="outline"
                  className="w-full h-11 gap-2 border-blue-500 text-blue-700 hover:bg-blue-50 text-base"
                >
                  <Navigation className="h-4 w-4" />
                  الموصل في الطريق (في الطريق إلى العميل 🛵)
                </Button>
              )}
              ...
              {order.status !== 'delivered' && (
                <Button
                  ...
                  disabled={updateOrderStatusMutation.isPending}
                  variant="outline"
                  className="w-full h-11 gap-2 border-emerald-500 text-emerald-700 hover:bg-emerald-50 text-base"
                >
                  <CheckCircle className="h-4 w-4" />
                  تأكيد تسليم الطلب (تم التسليم بنجاح ✅)
                </Button>
              )}
            </div>
          )}
          ...
          >
            ...
            اتصال والتواصل مع العميل
          </Button>
        </div>
      </main>

      {/* نافذة الاتصال والتواصل الآمنة المتوافقة مع أندرويد وواتساب */}
      <CallContactDialog
