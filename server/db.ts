...
export class DatabaseStorage {
  ...
  async findAdminByIdentifier(identifier: string): Promise<AdminUser | undefined> {
    ...
    // 1. Direct match by email, username, or exact phone
    const directMatch = await this.getAdminByEmail(cleanInput);
    if (directMatch) return directMatch;

    const phoneMatch = await this.getAdminByPhone(cleanInput);
    if (phoneMatch) return phoneMatch;

    // 2. Flexible phone digits matching
    if (inputDigits.length >= 6) {
      const allAdmins = await this.getAllAdminUsers();
...
async getNotifications(recipientType?: string, recipientId?: string, unread?: boolean): Promise<Notification[]> {
  ...
  async getCustomerWallets(): Promise<any[]> {
    try {
      const allWallets = await this.db.select().from(wallets);
  ...
  async getCustomerWallet(phone: string): Promise<any | undefined> {
    try {
      ...
      return {
        id: newWallet.id,
        customerPhone: newWallet.customerPhone,
        balance: String(newWallet.balance || "0.00"),
        loyaltyPoints: 0,
        isActive: newWallet.isActive ?? true,
  ...
  async createOrUpdateCustomerWallet(phone: string, data: any): Promise<any> {
    try {
      ...
      if (existing) {
        const [updated] = await this.db.update(wallets)
          .set({
  ...
  async getCustomerWalletTransactions(phone: string): Promise<any[]> {
    try {
      ...
      if (!wallet) return [];
      const txs = await this.db.select().from(walletTransactions)
  ...
  async createCustomerWalletTransaction(phone: string, tx: any): Promise<any> {
    try {
      ...
      await this.createOrUpdateCustomerWallet(phone, { balance: newBalance.toFixed(2) });

      const [newTx] = await this.db.insert(walletTransactions).values({
        walletId: wallet.id,
        type: tx.type || 'credit',
        amount: String(txAmount.toFixed(2)),
