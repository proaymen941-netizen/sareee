...
const defaultUiSettings = [
  ...
  { key: "enable_scheduled_orders", value: "false", category: "driver", description: "السماح للعملاء بجدولة الطلبات خارج ساعات الموصلين" },
  // إعدادات رموز التحقق OTP والواتساب
  { key: "enable_otp", value: "true", category: "otp", description: "تفعيل رمز التحقق عند تسجيل الحساب" },
  { key: "otp_environment", value: "development", category: "otp", description: "بيئة عمل التحقق OTP: تجربة وتطوير (development) أم إنتاج حقيقي (production)" },
  { key: "otp_channel", value: "whatsapp", category: "otp", description: "قناة إرسال رمز التحقق المفضلة" },
  { key: "whatsapp_access_token", value: "", category: "otp", description: "رمز الوصول لواتساب Meta Access Token" },
  { key: "whatsapp_phone_number_id", value: "1334025846455164", category: "otp", description: "معرّف رقم هاتف الواتساب Phone Number ID" },
  { key: "whatsapp_business_account_id", value: "2245913132898564", category: "otp", description: "معرّف حساب واتساب للأعمال WhatsApp Business Account ID" },
  { key: "otp_whatsapp_number", value: "+1 (555) 197-1367", category: "otp", description: "رقم مرسل الواتساب المعتمد" },
  { key: "whatsapp_template_name", value: "", category: "otp", description: "اسم قالب واتساب المعتمد (اختياري)" },
  { key: "otp_sms_provider_url", value: "", category: "otp", description: "رابط بوابة SMS" },
];
  ...
  try {
    ...
    for (const setting of defaultUiSettings) {
      if (!existingKeys.has(setting.key)) {
        await storage.createUiSetting(setting);
        added++;
      }
    }
    if (added > 0) {
...
export async function ensureAdminUsers() {
  try {
    ...
    for (const acc of adminAccounts) {
      const existing = (await storage.getAdminByEmail(acc.email)) || 
                       (await storage.getAdminByEmail(acc.username)) || 
                       (await storage.findAdminByIdentifier(acc.phone));
      if (!existing) {
        await storage.createAdminUser(acc);
        console.log(`👤 تم إنشاء حساب مدير النظام بنجاح: ${acc.name} (${acc.username} / ${acc.phone})`);
      } else {
        await storage.updateAdminUser(existing.id, {
          name: acc.name,
          email: acc.email,
        ...
        console.log(`👤 تم تحديث حساب مدير النظام وتأكيد كلمة المرور: ${acc.name} (${acc.username} / ${acc.phone})`);
      }
    }
  } catch (error) {
    console.error('خطأ في ضمان حسابات المديرين:', error);
  }
}
...
export async function seedDefaultData() {
  try {
    ...
    console.error('❌ Database seeding failed, skipping:', error);
  }
}