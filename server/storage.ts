...
export interface IStorage {
  ...
  // Wasalni
  getWasalniRequests(filters?: any): Promise<any[]>;
  getWasalniRequest(id: string): Promise<any | undefined>;
  createWasalniRequest(data: any): Promise<any>;
  updateWasalniRequest(id: string, data: any): Promise<any | undefined>;
  deleteWasalniRequest(id: string): Promise<boolean>;
  ...
  createDriverReview(review: any): Promise<any>;
  deleteDriverReview(id: string): Promise<boolean>;
  ...
  findAdminByIdentifier(identifier: string): Promise<AdminUser | undefined>;
  ...
  // Customer Wallets
  getCustomerWallets(): Promise<any[]>;
  getCustomerWallet(phone: string): Promise<any | undefined>;
  createOrUpdateCustomerWallet(phone: string, data: any): Promise<any>;
  getCustomerWalletTransactions(phone: string): Promise<any[]>;
  createCustomerWalletTransaction(phone: string, tx: any): Promise<any>;
}
...
export class MemStorage {
  ...
  private restaurantSections: Map<string, RestaurantSection>;
  ...
  private customerWalletsMap: Map<string, any>;
  private customerWalletTransactionsMap: Map<string, any[]>;
  ...
  private wasalniRequestsMap: Map<string, any>;
  private couponsMap: Map<string, any>;
  private couponUsagesMap: Map<string, any>;
  private driverReviewsMap: Map<string, any>;
  ...
  constructor() {
    ...
    this.restaurantSections = new Map();
    ...
    this.driverReviewsMap = new Map();
    this.paymentMethodsMap = new Map();
    this.paymentGatewaysMap = new Map();
  ...
  private initializeData() {
    ...
    categories.forEach(cat => this.categories.set(cat.id, cat as any));
    ...
    restaurants.forEach(restaurant => this.restaurants.set(restaurant.id, restaurant as any));
    ...
    menuItems.forEach(item => this.menuItems.set(item.id, item as any));
    ...
    drivers.forEach(driver => this.drivers.set(driver.id, driver as any));
    ...
    drivers.forEach(driver => {
      const balance: DriverBalance = {
        id: randomUUID(),
        driverId: driver.id,
        totalBalance: driver.earnings || "0",
        availableBalance: driver.earnings || "0",
        withdrawnAmount: "0",
        pendingAmount: "0",
        createdAt: new Date(),
        updatedAt: new Date()
  ...
  async updateDriverBalance(driverId: string, data: { amount: number; type: string; description: string; orderId?: any; }): Promise<DriverBalance> {
    ...
    let newTotalBalance = parseFloat(String(balance.totalBalance || "0"));
    let newAvailableBalance = parseFloat(String(balance.availableBalance || "0"));
    let newWithdrawnAmount = parseFloat(String(balance.withdrawnAmount || "0"));
    let newPendingAmount = parseFloat(String(balance.pendingAmount || "0"));
    ...
    const updatedBalance: DriverBalance = {
      ...
      withdrawnAmount: newWithdrawnAmount.toFixed(2),
    ...
    if (driver) {
      this.drivers.set(driverId, {
        ...driver,
        earnings: newTotalBalance.toFixed(2),
        updatedAt: new Date()
      });
    }
  ...
  async createDriverBalance(data: InsertDriverBalance): Promise<DriverBalance> {
    const balance: DriverBalance = {
      ...
      totalBalance: data.totalBalance ? String(data.totalBalance) : "0",
      availableBalance: data.availableBalance ? String(data.availableBalance) : "0",
      withdrawnAmount: data.withdrawnAmount ? String(data.withdrawnAmount) : "0",
      pendingAmount: data.pendingAmount ? String(data.pendingAmount) : "0",
      createdAt: new Date(),
      updatedAt: new Date()
    };
  ...
  async createDriverTransaction(data: Omit<DriverTransaction, 'id' | 'createdAt' | 'balanceBefore' | 'balanceAfter'>): Promise<DriverTransaction> {
    ...
    const prevBal = parseFloat(String(balance?.availableBalance || "0"));
    const numAmt = parseFloat(String(data.amount || "0"));
    const isAdditive = data.type === 'commission' || data.type === 'salary' || data.type === 'bonus';
    const newBal = isAdditive ? prevBal + numAmt : prevBal - numAmt;

    const transaction: DriverTransaction = {
      ...data,
      id,
      balanceBefore: prevBal.toFixed(2),
  ...
  async getUserByPhone(phone: string): Promise<User | undefined> {
    ...
    return Array.from(this.users.values()).find(user => user.phone && user.phone.replace(/\s+/g, '') === cleanPhone);
  }

  async findUserByIdentifier(identifier: string): Promise<User | undefined> {
    if (!identifier) return undefined;
    const idNoSpaces = identifier.replace(/\s+/g, '');
    const idLower = identifier.toLowerCase();
    return Array.from(this.users.values()).find(u => 
      (u.phone && (u.phone === identifier || u.phone.replace(/\s+/g, '') === idNoSpaces)) ||
      (u.username && (u.username === identifier || u.username.replace(/\s+/g, '') === idNoSpaces)) ||
      (u.name && (u.name === identifier || u.name.toLowerCase() === idLower)) ||
  ...
  async createUser(insertUser: InsertUser): Promise<User> {
    ...
    const user: User = { 
      ...
      name: insertUser.name || insertUser.username || "عميل",
  ...
  async getRestaurantSections(restaurantId: string): Promise<RestaurantSection[]> {
    return Array.from(this.restaurantSections.values())
  ...
  async createMenuItem(menuItem: InsertMenuItem): Promise<MenuItem> {
    ...
    const newMenuItem = { 
      ...
      sizes: menuItem.sizes ?? null,
      brand: menuItem.brand ?? null,
      rating: menuItem.rating ?? null,
      reviewCount: menuItem.reviewCount ?? null,
  ...
  async createOrder(order: InsertOrder): Promise<Order> {
    ...
    const newOrder = { 
      ...
      isWaselLi: order.isWaselLi ?? false,
      isScheduled: order.isScheduled ?? false,
      isScheduledOrderSent: order.isScheduledOrderSent ?? false,
  ...
  async completeOrder(orderId: string): Promise<Order | undefined> {
    try {
      ...
      if (order.driverId) {
        ...
        if (driverEarnings > 0) {
          await this.createDriverCommission({
            ...
            orderId: order.id,
            orderAmount: (order.totalAmount?.toString() || '0'),
            commissionRate: (order.driverCommissionRate?.toString() || '70'),
            commissionAmount: driverEarnings.toString(),
            status: 'approved'
          });
      ...
      console.error('Error completing order in MemStorage:', error);
      return undefined;
    }
  }
  ...
  async getWasalniRequests(filters?: any): Promise<any[]> {
    let requests = Array.from(this.wasalniRequestsMap.values());
    if (filters?.status) {
      requests = requests.filter(r => r.status === filters.status);
    }
    if (filters?.customerId) {
      requests = requests.filter(r => r.customerId === filters.customerId);
    }
    if (filters?.phone) {
      const target = String(filters.phone).trim().replace(/\s+/g, '');
      requests = requests.filter(r => String(r.customerPhone || '').trim().replace(/\s+/g, '') === target);
    }
  ...
  async createWasalniRequest(data: any): Promise<any> {
    ...
    this.wasalniRequestsMap.set(id, newReq);
    return newReq;
  }

  async updateWasalniRequest(id: string, data: any): Promise<any | undefined> {
    const existing = this.wasalniRequestsMap.get(id);
    if (!existing) return undefined;
    const updated = { ...existing, ...data, updatedAt: new Date() };
    this.wasalniRequestsMap.set(id, updated);
    return updated;
  }

  async deleteWasalniRequest(id: string): Promise<boolean> {
    return this.wasalniRequestsMap.delete(id);
  }
  ...
  async createDriver(driver: InsertDriver): Promise<Driver> {
    ...
    const newDriver: Driver = { 
      ...
      commissionRate: String(driver.commissionRate ?? "70"),
      averageRating: String(driver.averageRating ?? "0.00"),
      ...
      joinDate: driver.joinDate ?? new Date()
    };
  ...
  async updateDriver(id: string, driver: Partial<InsertDriver>): Promise<Driver | undefined> {
    ...
    if (driver.earnings !== undefined) {
      ...
      if (balance) {
        const earningsDiff = parseFloat(String(driver.earnings || "0")) - parseFloat(String(existing.earnings || "0"));
        if (earningsDiff !== 0) {
  ...
  async createSpecialOffer(offer: InsertSpecialOffer): Promise<SpecialOffer> {
    ...
    const newOffer = { 
      ...
      restaurantId: offer.restaurantId ?? null,
  ...
  async getDriverReviews(driverId?: string): Promise<any[]> {
    let reviews = Array.from(this.driverReviewsMap.values());
    if (driverId) {
      reviews = reviews.filter(r => r.driverId === driverId);
    }
  ...
  async createDriverReview(review: any): Promise<any> {
    ...
    this.driverReviewsMap.set(id, newReview);
    return newReview;
  }

  async deleteDriverReview(id: string): Promise<boolean> {
    return this.driverReviewsMap.delete(id);
  }
  ...
  async updateAdminUser(id: string, adminUser: Partial<InsertAdminUser>): Promise<AdminUser | undefined> {
    ...
    const updated: AdminUser = {
      ...existing,
      ...adminUser,
      password: adminUser.password !== undefined ? adminUser.password : existing.password,
    };
  ...
  async getAdminByEmail(emailOrUsername: string): Promise<AdminUser | undefined> {
    ...
    return Array.from(this.adminUsers.values()).find(admin => 
      (admin.email && admin.email.toLowerCase() === term) ||
      (admin.username && admin.username.toLowerCase() === term) ||
      (admin.phone && admin.phone === raw)
    );
  }
  ...
  async findAdminByIdentifier(identifier: string): Promise<AdminUser | undefined> {
    if (!identifier) return undefined;
    const arabicToLatin = (s: string) =>
      s.replace(/[\u0660-\u0669]/g, (d) => String(d.charCodeAt(0) - 0x0660))
       .replace(/[\u06F0-\u06F9]/g, (d) => String(d.charCodeAt(0) - 0x06F0));

    const cleanInput = arabicToLatin(identifier.trim());
    const term = cleanInput.toLowerCase();
    const inputDigits = cleanInput.replace(/\D/g, '');

    const allAdmins = Array.from(this.adminUsers.values());

    // 1. Direct match by email or username (case-insensitive) or exact phone
    const directMatch = allAdmins.find(admin =>
      (admin.email && admin.email.toLowerCase() === term) ||
      (admin.username && admin.username.toLowerCase() === term) ||
      (admin.phone && admin.phone === cleanInput)
    );
    if (directMatch) return directMatch;

    // 2. Flexible phone digits matching
    if (inputDigits.length >= 6) {
      const stripPrefix = (d: string) => {
        let s = d;
        if (s.startsWith('00967')) s = s.slice(5);
        else if (s.startsWith('967')) s = s.slice(3);
        else if (s.startsWith('0')) s = s.slice(1);
        return s;
      };

      const cleanInputDigits = stripPrefix(inputDigits);

      for (const a of allAdmins) {
        if (!a.phone) continue;
        const aDigits = arabicToLatin(a.phone).replace(/\D/g, '');
        if (!aDigits) continue;
        if (aDigits === inputDigits) return a;
        const cleanADigits = stripPrefix(aDigits);
        if (cleanADigits === cleanInputDigits) return a;
        if (cleanADigits.endsWith(cleanInputDigits) || cleanInputDigits.endsWith(cleanADigits)) return a;
      }
    }
  ...
  async createNotification(notification: InsertNotification): Promise<Notification> {
    ...
    if (wsManager) {
      ...
      if (notification.recipientType === 'customer' && notification.recipientId) {
        wsManager.sendToUser(notification.recipientId, 'NEW_NOTIFICATION', newNotification);
      } else if (notification.recipientType === 'driver' && notification.recipientId) {
        wsManager.sendToDriver(notification.recipientId, 'NEW_NOTIFICATION', newNotification);
      } else if (notification.recipientType === 'admin') {
        wsManager.sendToAdmin('NEW_NOTIFICATION', newNotification);
      }
  ...
  async getCoupons(): Promise<any[]> {
    return Array.from(this.couponsMap.values()).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  }

  async getCouponByCode(code: string): Promise<any | undefined> {
    const uppercaseCode = (code || '').toUpperCase();
    return Array.from(this.couponsMap.values()).find(c => c.code === uppercaseCode);
  }

  async createCoupon(couponData: any): Promise<any> {
    const id = couponData.id || randomUUID();
    const newCoupon = {
      id,
      ...couponData,
      ...
      restaurantId: couponData.restaurantId || null,
  ...
  async updateCoupon(id: string, couponData: any): Promise<any | undefined> {
    ...
    const updated = {
      ...existing,
      ...couponData,
      code: couponData.code ? couponData.code.toUpperCase() : existing.code,
      restaurantId: couponData.restaurantId !== undefined ? (couponData.restaurantId || null) : existing.restaurantId,
      categoryId: couponData.categoryId !== undefined ? (couponData.categoryId || null) : existing.categoryId,
      updatedAt: new Date(),
    };
  ...
  async validateCoupon(code: string, orderValue: number, userId?: string, userPhone?: string): Promise<any> {
  ...
  async useCoupon(couponId: string, data: any): Promise<void> {
    const id = randomUUID();
    this.couponUsagesMap.set(id, { id, ...data, createdAt: new Date() });
    const coupon = this.couponsMap.get(couponId);
    if (coupon) {
      coupon.usageCount = (coupon.usageCount || 0) + 1;
      this.couponsMap.set(couponId, coupon);
    }
  }

  // Payment Gateways
  async getPaymentGateways(): Promise<any[]> {
    return Array.from(this.paymentGatewaysMap.values());
  }

  async getActivePaymentGateways(): Promise<any[]> {
    return Array.from(this.paymentGatewaysMap.values()).filter((g: any) => g.isActive !== false);
  }

  async createPaymentGateway(gateway: any): Promise<any> {
    const id = gateway.id || randomUUID();
    const newGateway = { id, ...gateway, createdAt: new Date(), updatedAt: new Date() };
    this.paymentGatewaysMap.set(id, newGateway);
    return newGateway;
  }

  async updatePaymentGateway(id: string, gateway: any): Promise<any | undefined> {
    const existing = this.paymentGatewaysMap.get(id);
    if (!existing) return undefined;
    const updated = { ...existing, ...gateway, updatedAt: new Date() };
    this.paymentGatewaysMap.set(id, updated);
    return updated;
  }

  async deletePaymentGateway(id: string): Promise<boolean> {
    return this.paymentGatewaysMap.delete(id);
  }

  // Payment Methods
  async getPaymentMethods(): Promise<any[]> {
    if (this.paymentMethodsMap.size === 0) {
      const defaultMethods = [
        {
          ...
          provider: "cash",
          icon: "Banknote",
          description: "الدفع نقداً عند استلام الطلب من السائق",
          isActive: true,
          isOnline: false,
        ...
        },
        {
          id: "pm-2",
          name: "App Wallet",
          nameAr: "محفظة التطبيق",
          type: "wallet",
          provider: "internal",
          icon: "Wallet",
          description: "الدفع خصماً من رصيد محفظتك المتاحة في التطبيق",
          isActive: true,
          isOnline: true,
          requiresDocument: false,
          sortOrder: 2,
        ...
        },
        {
          id: "pm-3",
          name: "Mada / Card",
          nameAr: "بطاقة مدى / بطاقة ائتمانية",
          type: "card",
          provider: "mada",
          icon: "CreditCard",
          description: "الدفع عبر بطاقات مدى أو الفيزا والماستركارد",
          isActive: true,
          isOnline: true,
          requiresDocument: false,
          sortOrder: 3,
        ...
        },
        {
          id: "pm-4",
          name: "Bank Transfer",
          nameAr: "تحويل بنكي / محفظة إلكترونية",
          type: "bank_transfer",
          provider: "bank",
          icon: "Building2",
          description: "تحويل بنكي مباشر عبر الحسابات المعتمدة أو الخدمات المالية",
          isActive: true,
          isOnline: true,
      ...
      defaultMethods.forEach(m => this.paymentMethodsMap.set(m.id, m));
    }
  ...
  async getActivePaymentMethods(): Promise<any[]> {
    ...
    return all.filter((m: any) => m.isActive !== false);
  }

  async getPaymentMethod(id: string): Promise<any | undefined> {
    const all = await this.getPaymentMethods();
    return this.paymentMethodsMap.get(id);
  }

  async createPaymentMethod(method: any): Promise<any> {
    const id = method.id || randomUUID();
    const newMethod = {
      id,
      ...method,
      isActive: method.isActive !== false,
      isOnline: method.isOnline !== false,
  ...
  async updatePaymentMethod(id: string, method: any): Promise<any | undefined> {
  ...
  async getPaymentMethodDocuments(paymentMethodId: string): Promise<any[]> {
    return Array.from(this.paymentMethodDocumentsMap.values())
      .filter((d: any) => d.paymentMethodId === paymentMethodId);
  }

  async createPaymentMethodDocument(doc: any): Promise<any> {
    ...
    this.paymentMethodDocumentsMap.set(id, newDoc);
    return newDoc;
  }

  async updatePaymentMethodDocument(id: string, doc: any): Promise<any | undefined> {
    const existing = this.paymentMethodDocumentsMap.get(id);
    if (!existing) return undefined;
    const updated = {
      ...existing,
      ...doc,
    ...
    this.paymentMethodDocumentsMap.set(id, updated);
    return updated;
  }

  async deletePaymentMethodDocument(id: string): Promise<boolean> {
    return this.paymentMethodDocumentsMap.delete(id);
  }
  ...
  async getDetailedReport(filters: any): Promise<any> {
    ...
    const allOrders = Array.from(this.orders.values()).filter(o => {
  ...
  // ==================== دوال محافظ العملاء ====================
  async getCustomerWallets(): Promise<any[]> {
    // جلب جميع المحافظ مع التأكد من وجود محفظة لكل مستخدم
    const users = Array.from(this.users.values());
    for (const u of users) {
      if (u.phone && !this.customerWalletsMap.has(u.phone)) {
        this.customerWalletsMap.set(u.phone, {
          id: randomUUID(),
    ...
    // إذا لم تكن هناك محافظ بعد، نهيئ محافظ افتراضية
    if (this.customerWalletsMap.size === 0) {
      const defaultWallets = [
        { id: randomUUID(), customerPhone: "+967771234567", balance: "250.00", loyaltyPoints: 120, isActive: true, createdAt: new Date(), updatedAt: new Date() },
        { id: randomUUID(), customerPhone: "777146387", balance: "500.00", loyaltyPoints: 300, isActive: true, createdAt: new Date(), updatedAt: new Date() },
        { id: randomUUID(), customerPhone: "+967771112233", balance: "150.00", loyaltyPoints: 80, isActive: true, createdAt: new Date(), updatedAt: new Date() },
      ];
      defaultWallets.forEach(w => this.customerWalletsMap.set(w.customerPhone, w));
    }

    return Array.from(this.customerWalletsMap.values());
  }

  async getCustomerWallet(phone: string): Promise<any | undefined> {
    if (!phone) return undefined;
    const cleanPhone = phone.trim();
    let wallet = this.customerWalletsMap.get(cleanPhone);
    if (!wallet) {
      // بحث مرن برقم الهاتف
      wallet = Array.from(this.customerWalletsMap.values()).find(w => 
        w.customerPhone === cleanPhone || 
        w.customerPhone.endsWith(cleanPhone) || 
        cleanPhone.endsWith(w.customerPhone)
      );
    }
    if (!wallet) {
      wallet = {
      ...
      this.customerWalletsMap.set(cleanPhone, wallet);
    }
    return wallet;
  }

  async createOrUpdateCustomerWallet(phone: string, data: any): Promise<any> {
    const existing = await this.getCustomerWallet(phone);
    const updated = {
      ...existing,
      ...data,
      balance: data.balance !== undefined ? String(data.balance) : existing.balance,
      updatedAt: new Date()
    };
    this.customerWalletsMap.set(phone, updated);
    return updated;
  }

  async getCustomerWalletTransactions(phone: string): Promise<any[]> {
    if (!phone) return [];
    const cleanPhone = phone.trim();
    let txs = this.customerWalletTransactionsMap.get(cleanPhone);
    if (!txs) {
      // بيانات تجريبية أولية للمعاملات
      txs = [
        {
          id: randomUUID(),
          type: "credit",
          amount: "100.00",
          description: "هدية ترحيبية / شحن رصيد افتتاحي",
          orderId: undefined,
          createdAt: new Date(Date.now() - 3600000 * 24).toISOString()
        }
      ];
      this.customerWalletTransactionsMap.set(cleanPhone, txs);
    }
    return txs;
  }

  async createCustomerWalletTransaction(phone: string, tx: any): Promise<any> {
    const wallet = await this.getCustomerWallet(phone);
    const currentBalance = parseFloat(wallet.balance || "0");
    const txAmount = parseFloat(tx.amount || "0");
    
    let newBalance = currentBalance;
    if (tx.type === 'credit' || tx.type === 'loyalty') {
      newBalance += txAmount;
    } else if (tx.type === 'debit') {
      newBalance = Math.max(0, currentBalance - txAmount);
    }

    await this.createOrUpdateCustomerWallet(phone, {
      balance: newBalance.toFixed(2),
      loyaltyPoints: tx.type === 'loyalty' ? (wallet.loyaltyPoints || 0) + Math.round(txAmount) : wallet.loyaltyPoints
    });
    ...
    const newTx = {
      ...
      description: tx.description || 'معاملة محفظة',
      orderId: tx.orderId || null,
      createdAt: new Date().toISOString()
    };

    const txList = await this.getCustomerWalletTransactions(phone);
    txList.unshift(newTx);
    this.customerWalletTransactionsMap.set(phone, txList);

    return { transaction: newTx, wallet: await this.getCustomerWallet(phone) };
  }
}
...
function createSmartStorage(): IStorage {
  ...
  let useDb = !!(process.env.DATABASE_URL && process.env.DATABASE_URL.trim() !== "");

  const dummyDbProxy: any = new Proxy({}, {
    get(_target, prop) {
      if (prop === 'then') return undefined;
      return (..._args: any[]) => {
        const dummyQuery: any = new Proxy(Promise.resolve([]), {
          get(target, methodProp) {
            if (methodProp === 'then') return target.then.bind(target);
            if (methodProp === 'catch') return target.catch.bind(target);
            if (methodProp === 'finally') return target.finally.bind(target);
            return () => dummyQuery;
          }
        });
        return dummyQuery;
  ...
  return new Proxy(dbStorage as any, {
    get(target, prop, receiver) {
        ...
        return async function (...args: any[]) {
          if (useDb && typeof dbMethod === 'function') {
            try {
              ...
              console.warn(`⚠️ DB error in '${String(prop)}' (${err?.message}). Falling back to memory storage.`);
              useDb = false;
      ...
      return useDb ? Reflect.get(target, prop, receiver) : Reflect.get(mem, prop, receiver);
    }
  });
}
