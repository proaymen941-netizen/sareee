...
router.put("/profile", async (req: any, res) => {
  try {
    ...
    if (req.admin?.id) {
      targetAdmin = await storage.getAdminById(req.admin.id);
    }
    if (!targetAdmin) {
      const allAdmins = await storage.getAllAdminUsers();
      targetAdmin = allAdmins.find((u: any) => u.userType === 'admin') || allAdmins[0];
    }
    if (!targetAdmin) return res.status(404).json({ error: "لم يتم العثور على ملف المدير" });

    const cleanEmail = email.trim().toLowerCase();
    const cleanUsername = username && username.trim() ? username.trim() : null;
    const cleanPhone = phone && phone.trim() ? phone.trim() : null;

    // Check duplicate email / username against other admin users
    const allAdmins = await storage.getAllAdminUsers();
    const emailConflict = allAdmins.find(
      (u: any) => u.id !== targetAdmin.id && u.email && u.email.toLowerCase() === cleanEmail
    );
    if (emailConflict) {
    ...
    if (cleanUsername) {
      const usernameConflict = allAdmins.find(
        (u: any) => u.id !== targetAdmin.id && u.username && u.username.toLowerCase() === cleanUsername.toLowerCase()
      );
      if (usernameConflict) {
    ...
    const updated = await storage.updateAdminUser(targetAdmin.id, updatePayload);
...
router.put("/change-password", async (req: any, res) => {
  try {
    ...
    if (!admin) return res.status(404).json({ error: "لم يتم العثور على حساب المدير" });
    ...
    const updated = await storage.updateAdminUser(admin.id, { password: hashed });
    if (!updated) {
      return res.status(500).json({ error: "فشل حفظ كلمة المرور الجديدة" });
    }
...
router.get("/sub-admins", requirePermission('manage_admins'), async (req, res) => {
  try {
    let subAdmins: any[] = [];
    try {
      subAdmins = await db.select().from(adminUsers).where(eq(adminUsers.userType, 'sub_admin'));
    } catch (dbErr) {
      console.warn("⚠️ فشل جلب المشرفين من قاعدة البيانات، استخدام التخزين الاحتياطي:", dbErr);
      const all = await storage.getAllAdminUsers();
      subAdmins = (all || []).filter((u: any) => u.userType === 'sub_admin');
    }
    const safe = (subAdmins || []).map((u: any) => { const { password: _, ...rest } = u; return rest; });
    res.json(safe);
  } catch (error: any) {
    console.error("❌ خطأ في جلب المشرفين الفرعيين:", error);
    res.status(500).json({ error: "خطأ في جلب بيانات المشرفين" });
  }
});
...
router.post("/sub-admins", requirePermission('manage_admins'), async (req, res) => {
  try {
    ...
    let finalEmail = (email && email.trim()) 
      ...
      : `${cleanPhone || Date.now()}@subadmin.local`;
    ...
    try {
      const existingPhone = await storage.getAdminByPhone(trimmedPhone);
      if (existingPhone) {
        return res.status(409).json({ error: "رقم الهاتف مسجل بالفعل لمشرف آخر" });
      }
      if (finalEmail) {
        const existingEmail = await storage.getAdminByEmail(finalEmail);
        if (existingEmail) {
          finalEmail = `${cleanPhone || Date.now()}_${Math.floor(Math.random() * 10000)}@subadmin.local`;
        }
      }
    } catch (_) {}

    const hashed = await bcrypt.hash(password, 10);
    const permsString = typeof permissions === 'string' 
      ? permissions 
      : JSON.stringify(permissions || []);

    let newSubAdmin: any = null;
    ...
    try {
      ...
      if (inserted && inserted.length > 0) {
        newSubAdmin = inserted[0];
      }
    } catch (dbErr: any) {
      console.warn("⚠️ فشل db.insert للمشرف الفرعي، التبديل للتخزين الاحتياطي:", dbErr?.message || dbErr);
      if (dbErr?.code === '23505') {
        return res.status(409).json({ error: "رقم الهاتف أو البريد الإلكتروني مستخدم بالفعل" });
      }
    }

    // إذا لم يرجع الإدخال نتيجة أو حدث خطأ، استخدم storage كاحتياطي مضمون
    if (!newSubAdmin) {
      try {
        newSubAdmin = await storage.createAdminUser({
          name: name.trim(),
          email: finalEmail,
          username: finalUsername,
          phone: trimmedPhone,
        ...
        console.error("❌ فشل storage.createAdminUser للمشرف:", storageErr);
      }
    }

    if (!newSubAdmin) {
      return res.status(500).json({ error: "فشل في حفظ المشرف، يرجى مراجعة البيانات والمحاولة مرة أخرى" });
    }

    const { password: _, ...safe } = newSubAdmin as any;
    res.status(201).json(safe);
  } catch (error: any) {
    console.error("❌ خطأ غير متوقع عند إضافة مشرف فرعي:", error);
    if (error?.code === '23505') {
      return res.status(409).json({ error: "رقم الهاتف أو البريد الإلكتروني مستخدم بالفعل" });
    }
    res.status(500).json({ error: error?.message || "خطأ في الخادم أثناء إضافة المشرف" });
  }
});
...
router.put("/sub-admins/:id", requirePermission('manage_admins'), async (req, res) => {
  try {
    ...
    try {
      const [dbUpdated] = await db.update(adminUsers).set(updateData).where(eq(adminUsers.id, req.params.id)).returning();
      updated = dbUpdated;
    } catch (dbErr) {
      console.warn("⚠️ db.update فشل، التبديل للتخزين:", dbErr);
    }
    if (!updated) {
      updated = await storage.updateAdminUser(req.params.id, updateData);
    }
    if (!updated) return res.status(404).json({ error: "المشرف غير موجود" });
    const { password: _, ...safe } = updated as any;
    res.json(safe);
  } catch (error: any) {
    console.error("❌ خطأ في تعديل المشرف:", error);
    res.status(500).json({ error: "خطأ في الخادم أثناء تعديل المشرف" });
  }
});
...
router.delete("/sub-admins/:id", requirePermission('manage_admins'), async (req, res) => {
  try {
    try {
      ...
      console.warn("⚠️ db.delete فشل للمشرف، محاولة storage:", dbErr);
    }
    try {
    ...
    res.status(500).json({ error: "خطأ في الخادم أثناء حذف المشرف" });
  }
});
