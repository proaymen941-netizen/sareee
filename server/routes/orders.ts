...
import { autoAssignDriverToOrder } from "../utils/auto-assign.js";
...
// تعيين أو تغيير سائق للطلب (أو إعادته إلى قائمة المتاح للجميع)
router.put("/:id/assign-driver", async (req, res) => {
  try {
    ...
    const isUnassigning = !driverId || driverId === 'unassigned' || driverId === 'null';

    // 1. حالة إلغاء التعيين وإعادة الطلب متاحاً لجميع السائقين
    if (isUnassigning) {
      const prevDriverId = order.driverId;
      if (prevDriverId) {
        try {
          await storage.updateDriver(prevDriverId, { isAvailable: true });
          await storage.createNotification({
            type: 'order_unassigned',
            title: 'إلغاء تعيين الطلب',
            message: `تم إلغاء تعيينك للطلب رقم ${order.orderNumber} وأصبح متاحاً للجميع`,
            recipientType: 'driver',
            recipientId: prevDriverId,
            orderId: id,
            isRead: false
          });
        } catch (_) {}
      ...
      // بث للجميع في كل التطبيقات ليرن عند جميع السائقين
      broadcastEvent('new_order_available', storeOrderPayload);
      ...
      if (prevDriverId) {
        broadcastEvent('refresh_driver_orders', { driverId: prevDriverId, orderId: id });
      }
      ...
      if (ws) {
        ...
        if (prevDriverId && typeof ws.sendToDriver === 'function') {
          ws.sendToDriver(prevDriverId, 'refresh_driver_orders', { orderId: id });
        }
      }

      return res.json({
        success: true,
        message: "تم إلغاء التعيين وأصبح الطلب متاحاً لجميع السائقين",
        order: updatedOrder
      });
    }

    // 2. حالة تعيين أو تغيير السائق لسائق محدد
    const prevDriverId = order.driverId;
    ...
    if (!driver.isActive) {
      return res.status(400).json({ error: "حساب السائق غير مفعل" });
    }
    ...
    if (ws) {
      ...
      // إشعار مباشر للسائق الجديد مع بيانات الطلب
      if (typeof ws.sendToDriver === 'function') {
        ...
        if (prevDriverId) {
          ws.sendToDriver(prevDriverId, 'refresh_driver_orders', {
            orderId: id,
            message: `تم تحويل الطلب رقم ${order.orderNumber} لسائق آخر`
          });
        }
      }
    }
